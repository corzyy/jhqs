...
Rectangle {
    ...
    readonly property bool active: scope.dndActive
    readonly property bool compact: scope.tileSize("dnd") === "compact"
    ...
    scale: Theme.animationsEnabled && dndMouse.pressed ? 0.97 : 1.0
    ...
    Rectangle {
        ...
        opacity: dndMouse.containsMouse ? 0.08 : 0
    ...
    MouseArea {
        ...
        onPressAndHold: {
            dndMouse._holdAt = Date.now()
        }
        onClicked: {
            if (Date.now() - dndMouse._holdAt < 800) return
            scope.toggleDnd()
        }
    }
}
