// PanelKit — design primitives shared by the bar panels (Network, Weather,
// Volume), the control-center audio menu and BluetoothPanel. Each was
...
QtObject {
    ...
    // Pill toggle (Wi-Fi, °C/°F, mute). `offColor` drives the inactive
    // palette; `offBgAlpha` exists because the weather unit pill uses a
    // lighter inactive fill than the error-colored variants.
    component TogglePill: Rectangle {
        ...
        property real offBgAlpha: 0.16
        signal clicked()
        implicitWidth: pillLabel.implicitWidth + 24
        implicitHeight: 26
        radius: height / 2
        antialiasing: Theme.shapesAa
        color: pill.on ? Theme.withAlpha(pill.onColor, 0.16) : Theme.withAlpha(pill.offColor, pill.offBgAlpha)
