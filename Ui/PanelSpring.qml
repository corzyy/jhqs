...
// WeatherPanel, CalendarMenu, SystemTrayPanel, JhqsMenu all bind
...
Item {
    ...
    // Compat (WeatherPanel debug hook reads offset; 0 = open, 1 = closed).
    readonly property real offset: 1 - fade
    ...
    // slide/zoom/grow ride DefaultSpatial (500ms, [0.38,1.21,0.22,1]) —
    // the same curve the reference shell uses for every drawer
    // (offsetScale Behavior Anim). Grow overshoots slightly past 1, which
    // call sites read as the island bounce.
    ParallelAnimation {
        id: enterAnim
        ...
        NumberAnimation { target: root; property: "slideX"; to: 0; duration: Theme.panelAnimSlide; easing.type: Easing.BezierSpline; easing.bezierCurve: Theme.curveDefaultSpatial }
        NumberAnimation { target: root; property: "slideY"; to: 0; duration: Theme.panelAnimSlide; easing.type: Easing.BezierSpline; easing.bezierCurve: Theme.curveDefaultSpatial }
        NumberAnimation { target: root; property: "zoom"; to: 1; duration: Theme.panelAnimScale; easing.type: Easing.BezierSpline; easing.bezierCurve: Theme.curveDefaultSpatial }
        NumberAnimation { target: root; property: "grow"; to: 1; duration: Theme.durDefaultSpatial; easing.type: Easing.BezierSpline; easing.bezierCurve: Theme.curveDefaultSpatial }
